...
var logger = util.getLogger('capture', { label: 'Capture / Input' });



var procConfig = config.get('capture.proc');
...
var settingsModel = modSettings.getSettings('capture', {
    device: 'url',
...
function ffprobeInfo( input ) {
    ...
    ffprobe( settingsModel.get('url') , function(err, data){
        if( !_.isEmpty(err) ){
...
function startCapture() {
    ...
    if (captureModel.get('running') === true) {
        logger.debug('There is already a running capture process - pid: ', captureModel.get('processID'));
    ...
    logger.debug('spawned a new capture-process with pid %d - command: %s %s', captureProcess.pid, procConfig.name, spawnArgs.join(' '), JSON.stringify(spawnArgs));
