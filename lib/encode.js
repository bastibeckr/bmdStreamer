var debug = require('debug')('sat1-node-stream:encoder');
var winston = require('winston');
var config = require('config');
var ffmpeg = require('fluent-ffmpeg');
var PassThrough = require('stream').PassThrough;
var Backbone = require('backbone');
var Q = require('q');
var _ = require('lodash');

var PublicDataModel = require('./model').publicData;

var publicData = new PublicDataModel({
    name: 'encoder',
    running: false,
    command: '',
    progress: {}
});

var outputOpts = [
    '-profile:v main',
    '-preset medium',
    '-crf 29',
    '-tune zerolatency',
    '-g 30',
    '-ab 96000',
    '-ar 44100',
    '-ac 2'
];

var currentCommand;


function getBaseCommand(deferred) {
    var baseCommand = ffmpeg({
            'logger': winston
        })
        .on('start', function(cmd) {
            debug('Spawned Ffmpeg with command: ' + cmd);
            publicData.set({
                running: true,
                command: cmd
            });
        })
        .on('end', function() {
            debug('Encoder was successful.')
            deferred.resolve();
            publicData.set({
                running: false,
                command: ''
            });
        })
        .on('error', function(err, stdout, stderr) {
            debug('Encoder encountered an error.', err.message);
            winston.warn('Encoder encountered an error.', {
                'msg': err.message,
                'stderr': stderr
            });
            publicData.set({
                running: false,
                command: ''
            });
            deferred.reject(err.message);
        })
        .on('progress', function(prog) {
            publicData.set({
                progress: prog
            });
            deferred.notify(prog)
        });

    return baseCommand;
}

function startEncoding(inputObj) {

    debug('start encoding ');

    var deferred = Q.defer();

    if (typeof(inputObj) === 'object' && ('stdout' in inputObj)) {
        input = inputObj.stdout;
    }

    if (publicData.get('running') === true) {
        debug('won’t start encoding because encoder seems busy');
        deferred.reject('we already an active encoder.');
    }

    currentCommand = getBaseCommand(deferred);

    // var testStream = require('fs').createWriteStream('testStream.mp4');

    currentCommand.addInput(input)
        .inputOptions('-re')

    .addOutput('udp://192.168.178.43:1234')
        .videoCodec('libx264')
        .audioCodec('libmp3lame')
        .outputOptions(outputOpts)
        .videoFilters([{
            filter: 'format',
            options: ['yuv420p']
        }, {
            filter: 'scale',
            options: ['1280:720']
        }]).format('mpegts');





    currentCommand.run();

    return deferred.promise;
}


function encodePreview(inputObj) {
    debug('get preview');
    var deferred = Q.defer();

    if (typeof(inputObj) === 'object' && ('stdout' in inputObj)) {
        input = inputObj.stdout;
    }



    // if (publicData.get('running') === true) {
    //     debug('won’t start encoding because encoder seems busy');
    //     deferred.reject('we already an active encoder.');
    // }

    var command = ffmpeg()
        .on('start', function(cmd) {
            debug('PREVIEWSpawned Ffmpeg with command: ' + cmd);

        })
        .on('end', function() {
            debug('PREVIEWEncoder was successful.')
            deferred.resolve();

        })
        .on('error', function(err, stdout, stderr) {
            debug('PREVIEWEncoder encountered an error.', err.message);

            deferred.reject(err.message);
        })
        .addInput(input)
        .noAudio()
        .addInputOptions('-analyzeduration 5000M')
        .output('screenshot-2.png')
        .size('1280x720')
        .takeFrames(1)
        .run();

    return deferred.promise;
}

/**
 * [stopStreaming description]
 * @return {[type]} [description]
 */
function stopEncoding() {
    debug('Stop streaming?');
    if (currentCommand.kill) {
        debug('Found kill method');
        currentCommand.kill('SIGTERM');
    } else {
        debug('Didnt find kill method');
    }
}


function getInfo() {
    Q.nfcall(ffmpeg.getAvailableFormats).then(function(formats) {
        debug(formats);
    });
    Q.nfcall(ffmpeg.getAvailableCodecs).then(function(codecs) {
        debug(codecs);
    });
}


module.exports = {
    startEncoding: startEncoding,
    stopEncoding: stopEncoding,
    encodePreview: encodePreview,
    publicData: publicData,
};