...
Streaming.prototype.start = function(){
    ...
    if(  outputIsUrl ){
        ...
        if( inputIsUrl ){
            ...
            modEncoder.currentCommand.once('start', function(){
                deferred.resolve();
            });

            modEncoder.currentCommand.once('error', function(err){
                deferred.reject(err);
            });
...
Streaming.prototype.stop = function(){
    modPlayout.stopProcess().then(function(){
        logger.info('Playout successfully stopped.')
    });
    return modEncoder.stopEncoding();
}
