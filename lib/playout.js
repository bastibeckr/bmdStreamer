...
var settingsModel = new models.SettingsModel({}, {name: 'playout'});
...
function onStdOutClose() {
    playoutLogger.info('onStdOutClose', arguments);
}
...
function onProcessExit(code, signal) {
    ...
    if (code > 0) {
        playoutLogger.warn('process exited with code > 0: ', {
            'code': code,
            'signal': signal,
            'stdErr': stdErr
        });
    } else {
        playoutLogger.info('process exited: ', {
            'code': code,
            'signal': signal
        });
    }
    playoutLogger.info('onProcessExit', arguments);
    // debug('proc after exit: ', captureProcess);
}
...
function getBmdPlayProcess(){
    ...
    playoutLogger.info('will spawn process:', spawnArgs.join(' '));
