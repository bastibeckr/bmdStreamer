...
var logger = util.getLogger('playout', { label: 'Playout / Output' });
...
var settingsModel = modSettings.getSettings('playout');
...
function generateParams(){
    ...
    logger.debug('Generate Parameters', settingsModel.toJSON());
    // Get device ID.
...
function getBmdPlayFormats(){
    ...
    proc.on('close', function(code){
        ...
        if( !_.isArray(devices) || _.isEmpty(devices) ){
            ...
            logger.debug('BMD Play found ' + devices.length + ' devices.');
...
function getBmdPlayProcess(){
    ...
    logger.debug('will spawn process:', spawnArgs.join(' '));
